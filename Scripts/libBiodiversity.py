#Written by Aurelien Pelissier - last update on 02/2020
...
def Shannon_entropy_Chao(sample):
    ...
    if np.min(ki) < 1:
        denormalize_factor = int(1/np.min(ki))
        sample = {k: v*denormalize_factor for k, v in sample.items()} #Sample should not be normalized to compute Shanon entropy
        ki = np.array(list(sample.values()))
    Ntot = np.sum(np.array(list(sample.values())).astype(np.float))
    f1 = np.size(np.where(ki==1))
    C = 1 - f1/Ntot
